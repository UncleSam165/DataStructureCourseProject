...
void GUI::PrintMessage(string msg) const	//Prints a message on status bar
{
	...
	pWind->DrawString(10, WindHeight - (int) (StatusBarHeight/1), msg); // You may need to change these coordinates later 
...
void GUI::UpdateStatusBar(int CurrentTimeStep, double CH, ENMY_STATUS CS, int ActiveCount, int ActiveFighters, int ActiveHealers, int ActiveFreezers,
	int FrostedCount, int FrostedFighters, int FrostedHealers, int FrostedFreezers,
	int KilledCount, int KilledFighters, int KilledHealers, int KilledFreezers)
{
	ClearStatusBar();
	//print current timestep
	char Buffer[300];
	char Cstate[7];
	if (CS == ACTV) { sprintf(Cstate, "Active"); }
	else if (CS == FRST) { sprintf(Cstate, "Frosted"); }
	sprintf(Buffer, "Time Step %i", CurrentTimeStep);
	PrintMessage(Buffer);
	sprintf(Buffer, "Castle Health: %f    Castle State: %s", CH, Cstate);
	PrintMessage(Buffer,1.2);
	sprintf(Buffer, "Active Count: %i    Active Fighter: %i    Active Healers: %i    Active Freezers: %i", ActiveCount, ActiveFighters, ActiveHealers, ActiveFreezers);
	PrintMessage(Buffer, 1.4);
	sprintf(Buffer, "Frosted Count: %i    Frosted Fighter: %i    Frosted Healers: %i    Frosted Freezers: %i", FrostedCount, FrostedFighters, FrostedHealers, FrostedFreezers);
	PrintMessage(Buffer, 1.8);
	sprintf(Buffer, "Killed Count: %i    Killed Fighter: %i    Killed Healers: %i    Killed Freezers: %i", KilledCount, KilledFighters, KilledHealers, KilledFreezers);
	PrintMessage(Buffer, 3);

}

void GUI::UpdateInterface(int CurrentTimeStep, double CH, ENMY_STATUS CS,
	int ActiveCount, int ActiveFighters, int ActiveHealers, int ActiveFreezers,
	int FrostedCount, int FrostedFighters, int FrostedHealers, int FrostedFreezers,
	int KilledCount, int KilledFighters, int KilledHealers, int KilledFreezers)
{
	ClearDrawingArea();
	UpdateStatusBar(CurrentTimeStep, CH, CS, ActiveCount, ActiveFighters, ActiveHealers, ActiveFreezers,
...
void GUI::AddToDrawingList(const Enemy* pE)
{
	...
	int eType = static_cast<int>(pE->GetType());	
